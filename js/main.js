...
function main() {
	...
	//Make new object for controler
	let controler = new Controller(game);
	controler.begin(ctxCanvas);
	game._loadPromise.then(
		(result) => {
			...
			game._objects["Field"] = [field];
			field.craeteTiles(2,game._resources.imgTile);
			field.linkTiles();
			game._objects["Tile"] = field._arrayTilesForDraw;
			...
			render.drawAll(objectsForDraw, ctxCanvas,15);
			console.log(game);
			console.log(field);
		},
		(error) => {
